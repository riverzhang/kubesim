...
func getMainFailReason(message string) *SchedulerEmulatorReviewScheduleFailReason {
	slicedMessage := strings.Split(message, "\n")
	colon := strings.Index(slicedMessage[0], ":")

	fail := &SchedulerEmulatorReviewScheduleFailReason{
		Type:    slicedMessage[0][:colon],
		Message: strings.Trim(slicedMessage[0][colon+1:], " "),
	}
	return fail
}
...
func parsePodsReview(templatePods []*v1.Pod, status Status) []*SchedulerEmulatorReviewResult {
	templatesCount := len(templatePods)
	result := make([]*SchedulerEmulatorReviewResult, 0)

	for i := 0; i < templatesCount; i++ {
		result = append(result, &SchedulerEmulatorReviewResult{
			ReplicasOnNodes: make([]*ReplicasOnNode, 0),
			UsName:          templatePods[i].Name,
		})
	}
...
func getReviewSpec(podTemplates []*v1.Pod, desiredReplicas int32) SchedulerEmulatorReviewSpec {

	podCopies := make([]v1.Pod, len(podTemplates))
	deepCopyPods(podTemplates, podCopies)
	return SchedulerEmulatorReviewSpec{
		//	Templates:       podCopies,
...
func getReviewStatus(pods []*v1.Pod, status Status) SchedulerEmulatorReviewStatus {
	return SchedulerEmulatorReviewStatus{
		CreationTimestamp: time.Now(),
		Replicas:          int32(len(status.Pods)),
		Reason:            getMainFailReason(status.StopReason),
		Pods:              parsePodsReview(pods, status),
	}
}

func GetReport(pods []*v1.Pod, status Status, desiredReplicas int32) *SchedulerEmulatorReview {
	return &SchedulerEmulatorReview{
		Spec:   getReviewSpec(pods, desiredReplicas),
...
func SchedulerEmulatorReviewPrettyPrint(r *SchedulerEmulatorReview, verbose bool) {
	...
	if verbose {
		fmt.Printf("\nTermination reason: %v: %v\n", r.Status.Reason.Type, r.Status.Reason.Message)
	}
...
func SchedulerEmulatorReviewPrint(r *SchedulerEmulatorReview, verbose bool, format string) error {
	switch format {
	case "json":
		return SchedulerEmulatorReviewPrintJson(r)
	case "yaml":
		return SchedulerEmulatorReviewPrintYaml(r)
	case "":
		SchedulerEmulatorReviewPrettyPrint(r, verbose)
		return nil
	default:
